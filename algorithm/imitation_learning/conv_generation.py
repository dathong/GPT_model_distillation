...
model_stg_prob_mapping = {0:1,1:0.6,2:0.3,3:0}
if params['train'] != 'True':
    model_stg_prob_mapping = {0:0,1:0,2:0,3:0,4:0}
response_mapping = {}
student_agent = params['student_model']
# teacher_agent = "meta-llama/Llama-2-7b-chat-hf"
# teacher_agent = "gpt-4"
    ...
    while retry < 4:
        try:
            ...
            for _j in range(repeat_time):
                ...
                if p < model_stg_prob_mapping[int(stage_no)]:
                    ...
                    if int(stage_no) > 0:  # only for llama 2
                        agent_sys_txt += my_prompts.AGENT_GUIDELINE_PROMPT1
                        ...
                        d_agent['messages'][-1]['content'] = last_txt
...
async def my_main():
    ...
    # sys.exit()
    pickle.dump(response_mapping, open(os.path.join("stage" + stage_no,"response_mapping.pkl"), "wb"))
    pickle.dump(results,open(os.path.join("stage" + stage_no,"results.pkl"),"wb"))

    conv_res = [e[0] for e in results]


    fields = ["agent_prompt", "customer_prompt", "personality", "emotion", "difficulty", "conv"]
    with open(os.path.join("stage" + stage_no,"full_conversations.csv"), 'w', encoding="utf-8", newline='') as csvfile:
