...
file_to_eval = "gpt4_eval_{model}_{method}_{prompt}.pkl".format(
    model=params['model'],
...
def main():
    ...
    file_to_eval = "l3_70b_eval_{model}_{method}_{prompt}.pkl".format(
        model = params['model'],
        method= params['method'],
        prompt= params['prompt']
    )
    # print("file_to_eval = ",file_to_eval)
    results = pickle.load(open(os.path.join("checkpoints","table7",file_to_eval),'rb'))
    ...
    if params['use_api'] == 'True':
        ...
        file_to_eval = "full_conv_{model}_{method}.csv".format(
            model=params['model'],
            method=params['method'],
            prompt=params['prompt']
        )
